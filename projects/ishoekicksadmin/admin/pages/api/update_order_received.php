...
$sql = "UPDATE bookings 
        ...
        AND tracking_status = 'Confirmation'";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        ...
        $update_tracking = "UPDATE trackings SET tracking_status = 'Completed', updated_time = NOW() WHERE booking_id = ?";
        ...
        if ($check_result->num_rows > 0) {
            ...
            echo json_encode([
                "success" => false,
                "message" => "Booking status is currently: " . $row['tracking_status'] . ". Must be in 'Confirmation' status to mark as completed."
            ]);
        } else {
            echo json_encode([
                "success" => false,
